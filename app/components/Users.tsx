    ...
    <div className="border border-[#D7D7D7] rounded-lg px-4 bg-[#FFFFFF]">            
      ...
      {assignees && assignees.length > 0 ? (        
        assignees.map((assignee: user_data, index: number) => (            
          ...
          >
            <div className="text-[#546FFF] font-semibold">{assignee.f_name }</div>


            <div className="font-semibold">{assignee.email}</div>
            ...
            {isAdmin && (
              <div className="flex gap-3 items-center">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    handleUserDelete(assignee.email!);
                  }}
