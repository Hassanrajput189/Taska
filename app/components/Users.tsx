...
const Users = () => {
  ...
  // Disable user
  const handleUserDisable = async (email: string) => {
    try {
  ...
  return (
    <div className="border border-[#D7D7D7] rounded-lg px-4 bg-[#FFFFFF]">
      {/* Table Header */}
      <div className="grid grid-cols-1 sm:grid-cols-[3fr_3fr_2fr_2fr] items-center p-3 text-[#656F7D]">
        <div>Name</div>
        <div>Email</div>
        <div>Role</div>
        <div>Actions</div>
      ...
      {users && users.length > 0 ? (
        users.map((user: user_data, index: number) => (
          <div
            ...
            className="grid grid-cols-1 sm:grid-cols-[3fr_3fr_2fr_2fr] items-center p-4 border-t border-[#D7D7D7] cursor-pointer gap-2"
          >
            {/* Name */}
            <div className="text-[#546FFF] font-semibold">
              {user.f_name}
            </div>
